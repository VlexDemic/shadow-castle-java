...
public class Lobby {
    ...
    public Lobby() {
        // Только ближнее оружие (лук убран, теперь он вспомогательный)
        WeaponType[] weapons = {
                WeaponType.DAGGER, WeaponType.SWORD, WeaponType.SPEAR, WeaponType.AXE
        };
        ...
        for (int i = 0; i < weapons.length; i++) {
            ...
            weaponPedestals.add(new WeaponPedestal(
                    x, y, weapons[i], weaponNames[i], descriptions[i], damages[i]
            ));
        }
    }
    ...
    private void renderUI(Graphics2D g, int screenWidth, int screenHeight, Player player) {
        ...
        // Информация о луке (вспомогательное оружие)
