...
public class RedSlime extends Enemy {
    public RedSlime(float x, float y) {
        super(x, y, 30, 1, 0f);
    }
    ...
    protected void performAttack() {
        if (player == null) return;
        Projectile p = new Projectile(x, y, player.getX(), player.getY(), damage, Color.RED);
        projectiles.add(p);
        shootCooldown = 1.2f;
    }
    ...
    public void render(Graphics2D g, int screenX, int screenY) {
        ...
        if (isCharging) {
            ...
            g.setColor(new Color(255, 100, 100, (int)(150 * intensity)));
            g.fillOval(screenX + 10, screenY - 10, 15, 15);
        }
