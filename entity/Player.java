...
public class Player extends Entity {
    ...
    public void update(float delta, InputHandler input, Room currentRoom) {
        ...
        for (int i = 0; i < arrows_list.size(); i++) {
            ...
            a.update(delta, this, currentRoom);
            if (!a.isActive()) {
        ...
        if (!currentRoom.isWall((int)newX, (int)newY)) {
            x = newX;
            y = newY;
        }

        x = Math.max(0.2f, Math.min(19.8f, x));
        y = Math.max(0.2f, Math.min(14.8f, y));

        if (input.isLeftMouseJustPressed() && attackCooldown <= 0) {
