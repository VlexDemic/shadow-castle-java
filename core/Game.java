...
public class Game implements Runnable {
    ...
    private void update(float delta) {
        ...
        switch (state) {
            case MAIN_MENU:
                int selected = mainMenu.update(input, tutorialCompleted);
                if (selected == 0) {
                    // Новая игра (доступна только после обучения)
            ...
            case EXPLORING:
                ...
                minimap.updateMap(dungeon, currentRoom);
                minimap.markRoomVisited(currentRoom.getGridX(), currentRoom.getGridY(), currentRoom.isCleared());
    ...
    private void startTutorial() {
        ...
        // Запуск обучающего этажа
        player = new Player(10, 10, WeaponType.SWORD);
        currentFloor = 0; // Специальный этаж для обучения
        loadTutorial();
        state = GameState.EXPLORING;
    }

    private void loadTutorial() {
        dungeon = new DungeonGenerator(0, true); // true = обучающий режим
        ...
        minimap.reset();
        for (Room room : dungeon.getAllRooms().values()) {
            minimap.addRoom(room.getGridX(), room.getGridY(), room.getType());
        }
        minimap.markRoomVisited(currentRoom.getGridX(), currentRoom.getGridY(), currentRoom.isCleared());
    }
    ...
    private void completeTransition() {
        ...
        if (nextRoom != null) {
            ...
            switch (targetDirection) {
                case "UP": player.setPosition(10, 13); break;
                case "DOWN": player.setPosition(10, 2); break;
                case "LEFT": player.setPosition(18, 7.5f); break;
                case "RIGHT": player.setPosition(2, 7.5f); break;
            }
    ...
    private void render() {
        ...
        switch (state) {
            case MAIN_MENU:
                mainMenu.render(g, canvas.getWidth(), canvas.getHeight(), tutorialCompleted);
                break;
            case SETTINGS:
            ...
            case EXPLORING:
                ...
                player.setCameraPosition(camera.getCamX(), camera.getCamY());
                hud.render(g, player, currentFloor, canvas.getWidth(), canvas.getHeight());
                minimap.render(g, canvas.getWidth(), canvas.getHeight());
    ...
    private void loadFloor(int floor) {
        dungeon = new DungeonGenerator(floor, false);
        currentRoom = dungeon.getStartRoom();
        currentRoom.setPlayer(player);
        player.setPosition(10, 10);
        isTransitioning = false;
